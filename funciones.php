<?php
	...
	function cosenoAngulo( $consulta )
	{
		global $trans;
		global $resultados;
		productoEscalar( $consulta );
		for ( $i = 0; $i < count($trans); $i++ )
		{
			$sumX = 0;
			$sumY = 0;
			for ( $j = 0; $j < count($trans[0] ); $j++ )
			{
				$sumX += $trans[$i][$j]*$trans[$i][$j];
				$sumY += $consulta[$i]*$consulta[$i];
			}
			if ( $sumX * $sumY != 0 )
				$resultados[$i]= $resultados[$i]/sqrt( $sumX * $sumY );
			else $resultados[$i]=0;
		}
	}

	function coeficienteDice( $consulta )
	{
		global $trans;
		global $resultados;
		productoEscalar( $consulta );
		...
		for ( $i = 0; $i < count($trans); $i++ )
		{
			...
			$resultados[$i]= (2 * $resultados[$i] ) / ( $sumX + $sumY );
		}
	}

	function coeficienteJaccard( $consulta )
	{
		global $trans;
		global $resultados;
		productoEscalar( $consulta );
		for ( $i = 0; $i < count($trans); $i++ )
		{
			...
			$resultados[$i]= ( $resultados[$i] ) / ( $sumX + $sumY - $resultados[$i] );
		}
	}
